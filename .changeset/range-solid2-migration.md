...
- `repeat` and `<Repeat>` are **deprecated** — Solid 2.0 ships an equivalent built-in `<Repeat count={n}>` in `@solidjs/web`. See the README for migration guidance. The primitives remain functional for cases that require incremental child creation/disposal.
