...
export type PropOptions<T> =
  (T extends undefined ? { initialValue?: T } : { initialValue: T }) &
  {
...
export const BoolProp: Component<PropProps<boolean>> = (props) => <label>
  <input
    type="checkbox"
    name={props.name}
    checked={props.value()}
...
export const NumberProp: Component<PropProps<number>> = (props) => <label>
  <span>{props.name}</span>
  {" "}
  <input
    type="number"
...
export const StringProp: Component<PropProps<string>> = (props) => <label>
  <span>{props.name}</span>
  {" "}
  <input
    type="text"
...
const filterEnum = <O extends Record<string, any>>(options: O): [key: string, value: number | string][] => {
  const entries = Object.entries(options);
  if (Object.keys(options).every((key) => key === options[options[key] as keyof O].toString())) {
    return entries.reduce((items, [key, value]) => {
      if (!items.includes(key)) { items.push(value) }
      return items;
    }, [] as string[]).map((item) => [item, options[item]]);
  }
  console.log(':(')
  return entries;
}
...
export const SelectProp = <T extends any>(
  ...
  return <label>
    ...
    <select
      ...
      onChange={(ev) => props.setValue(
        options()[ev.currentTarget.selectedIndex][1] as any
      )}
    >
 ...
 * ```
 */
export function createProp<T extends undefined>(name: string, options?: PropOptions<T>): PropReturn<T>;
...
export function createProp<T>(
  name: string,
  options: T extends undefined ? never
    : T extends object ? PropOptions<T> : PropOptions<T> | T
): PropReturn<T>;
export function createProp<T>(name: string, options?: PropOptions<T> | T): PropReturn<T> {
  const initialValue: T | undefined = options == null
    ? undefined
    : typeof options !== 'object'
      ? options as T
      : (options as PropOptions<T>).initialValue as T | undefined ??
      defaultInitialValues[
      ((options as PropOptions<T>).type as keyof typeof defaultInitialValues | undefined) ?? 'object'
      ] as T | undefined;
...
export type CreateProps =
  <Props extends { [name: string]: boolean | number | string | PropOptions<boolean | number | string | object> }>
    (props: Props) => [
      props: {
        ...
        [name in keyof Props as `set${Capitalize<name & string>}`]:
        Setter<Props[name] extends object ? Props[name]['options'] extends object ? Props[name]['options'][keyof Props[name]['options']] : Props[name]['initialValue'] : Props[name] extends boolean ? boolean : Props[name]>;
      },
      fields: JSX.Element[]
    ];
...
export const createProps: CreateProps = (props) =>
  Object.entries(props).reduce((result, [name, options]) => {
