      ...
      <div class="wrapper-v">
        ...
        <div class="chat-messages" ref={setMessageBox}>
          <For each={messages} fallback="no messages yet">
            {(message) => <div>{message instanceof Blob && message.type.startsWith("image/")
              ? <img src={URL.createObjectURL(message)} />
              : message.toString()}</div>}
          </For>
        </div>
        <form class="flex flex-row" onSubmit={(e) => {
          e.preventDefault();
          ...
          <button class="px-2" type="submit">Send</button>
        </form>
        <p class="fontsize-11">This demo chat is using a free testing service from <a href="https://socketsbay.com" target="_blank" rel="noreferrer,noopener">SocketsBay</a>.</p>
      </div>
    </div>
  );
};
