  ...
  return (
    <div class="p-2 box-border w-full min-h-screen grid grid-rows-3 justify-center space-y-4 bg-gray-800 text-white">
      ...
      <EmailForm
        ...
        onSend={evt => {
          evt.preventDefault()
          sendEmailAlternative(evt.detail)
            .then(console.log)
        }}
      />
    </div>
...
const Counter: Component<{
  ...
  return (
    <div class="wrapper-v w-3xl">
      ...
      <div class="flex">
        <button class="btn w-25 text-4xl" onClick={() => dispatch('valueChange')}> + </button>
        <button class="btn w-25 text-4xl" onClick={() => dispatch('valueChange', -1)}> - </button>
      </div>
        <button class="btn w-xs text-4xl" onClick={() =>dispatch('cleanCounter')}>Clear</button>
    </div>
  )
}
