...
const App: Component = () => {
  ...
  const serializeHTMLNode = (node: Node | null) => node === null
    ? 'null'
  ...
  createEffect(() => {
    ...
    if (start < 0 || start > 4 || end < 0 || end > 4) { setSelection([node, NaN, NaN]); }
  ...
  return (
    <div class="p-24 box-border w-full min-h-screen flex flex-col justify-center items-center space-y-4 bg-gray-800 text-white">
      <div class="wrapper-v items-start">
        ...
        <div>
          <h5>Selected:</h5>
          <span>{serializeHTMLNode(selection()[0])} {selection()[1]}-{selection()[2]}</span>
        </div>
        <div>
          <h5>Manipulate selection</h5>
          <select onChange={(ev) => { setNodeSelector(ev.currentTarget.value) }}>
          ...
          <input type="number" min="-1" max="4" value="-1" oninput={(ev) => setSelectionStart(ev.currentTarget.valueAsNumber)} />{'-'}
          <input type="number" min="-1" max="4" value="-1" oninput={(ev) => setSelectionEnd(ev.currentTarget.valueAsNumber)} />{' '}
        </div>
    </div>
  );
};
