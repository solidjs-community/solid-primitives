...
- `pan` — fires on single-pointer drag movement within the element bounds
- `pinch` — fires on two-pointer pinch/spread gestures with scale
- `rotate` — fires on two-pointer rotation gestures with angle
- `swipe` — fires when a fast directional swipe is detected
- `tap` — fires on a short, stationary pointer interaction
...
Multiple gesture refs can be combined using an array:
...
Calls `callback` continuously as a single pointer moves within the element.
...
Calls `callback` as two pointers move closer or farther apart. `scale` is relative to the initial two-finger distance when the second pointer was placed.
...
Calls `callback` as two pointers rotate around their center. `rotation` is in degrees relative to the initial angle when the second pointer was placed, clamped to –180..180.
...
type SwipeProps = {
  ...
  parameters?: {
    timeframe?: number;        // max ms for gesture to complete (default: 300)
...
Calls `callback` when a pointer is pressed and released without significant movement.
...
type TapProps = {
  ...
  maximumTapLength?: number; // max ms the pointer may be held (default: none)
};
```
